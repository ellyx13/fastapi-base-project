...
class BaseControllers:
    ...
    async def get_all(self, query: dict = None, search: str = None, search_in: list = None, page: int = 1, limit: int = 20, fields_limit: list = None, sort_by: str = 'created_at', order_by: str = 'desc', include_deleted: bool = False) -> dict:
        ...
        results = await self.service.get_all(query=query, search=search, search_in=search_in, page=page, limit=limit, fields_limit=fields_limit, sort_by=sort_by, order_by=order_by, include_deleted=include_deleted)
        return results
