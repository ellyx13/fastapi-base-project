...
class ErrorCode:
    ...
    def InvalidObjectId(_id: str):
        return CustomException(
            type="core/info/invalid-object-id",
            status=400,
            title="Invalid ID format.",
            detail=f"The id {_id} is not a valid object id. Please provide a valid object id and try again."
        )

    @staticmethod
    def InvalidEmail(email: str):
        return CustomException(
            type="core/info/invalid-email",
            status=400,
            title="Invalid email format.",
            detail=f"The {email} is not a valid email. Please provide a valid email and try again."
        )

    @staticmethod
    def InvalidPhone(phone: str):
        return CustomException(
            type="core/info/invalid-phone",
            status=400,
            title="Invalid phone format.",
            detail=f"The {phone} is not a valid phone. Please provide a valid phone with 10 number and try again."
        )

    @staticmethod
    def InvalidDate(date: str):
        return CustomException(
            type="core/info/invalid-date",
            status=400,
