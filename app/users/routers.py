...
from core.schemas import PaginationParams, ObjectIdStr
...
class RoutersCBV:
    ...
    async def get_all(self, pagination: PaginationParams = Depends()):
        ...
        results = await user_controllers.get_all(query=pagination.query, search=pagination.search, search_in=search_in, page=pagination.page, limit=pagination.limit, fields_limit=pagination.fields, sort_by=pagination.sort_by, order_by=pagination.order_by)
    ...
    @router.post("/users/register", status_code=201, responses={201: {"model": schemas.Response, "description": "Register user success"}})
    ...
    @router.post("/users/login", status_code=201, responses={201: {"model": schemas.Response, "description": "Register user success"}})
